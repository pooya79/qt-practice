...
Hex::Hex(QGraphicsItem *parent)
{
    ...
    // create a polygon with scaled points
...
{
    // if this hex is NOT placed then pick it up
    if (getIsPlaced() == false)
    {
        game->pickUpCard(this);
    }
    // if this hex IS placed, then replace it
    else if (game->cardToPlace && owner == QString("NOONE"))
    {
        game->placeCard(this);
    }
}
...
void Hex::createLines()
{
    ...
    QPointF finalPt(hexCenter.x(), hexCenter.y()-70);
...
void Hex::findNeighbors()
{
    for (size_t i = 0, n = lines.size(); i<n; i++)
    {
        ...
        if (neighbors.size() < i + 1)
        {
            Hex* item = new Hex(this);
            item->virt = true;
            neighbors.append(item);
        }
    }
...
void Hex::captureNeighbors()
{
    ...
    for (size_t i = 0, n = neighbors.size(); i<n; i++)
    {
        ...
        if (neighbors[i]->getOwner() != QString("NOONE"))
        {
            isNotNeutral = true;
        }
